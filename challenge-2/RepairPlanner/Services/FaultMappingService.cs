...
namespace RepairPlanner.Services
{
    ...
    // - '??' means "if null use the right-hand side" (like Python's 'or' for None-coalescing).
    // - '??=' means "assign if null" (like: x = x or default_value).
    // - Primary constructors make constructor parameters into fields (not used here).
    ...
    public sealed class FaultMappingService : IFaultMappingService
    {
        ...
        private static readonly IReadOnlyDictionary<string, IReadOnlyList<string>> FaultToParts =
            new Dictionary<string, IReadOnlyList<string>>(StringComparer.OrdinalIgnoreCase)
            {
                ...
                // intentionally empty array for faults that don't require parts
